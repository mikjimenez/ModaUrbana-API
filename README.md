...
```env
MONGODB_URI=mongodb+srv://usuario:password@cluster.mongodb.net/modaurbana_db
...
El servidor estará disponible en: `http://localhost:3012`
...
2. **Reemplazar** `{{id}}` en la URL con el ID copiado:
   ```
   http://localhost:3012/api/producto/507f1f77bcf86cd799439011/upload-image
   ```
...
**Imagen original:**
```
http://localhost:3012/uploads/1700000000000-imagen.jpg
```

**Thumbnail (miniatura):**
```
http://localhost:3012/uploads/thumbnails/thumb-1700000000000-imagen.jpg
```
...
TOKEN=$(curl -X POST http://localhost:3012/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"productor@example.com","password":"password123"}' \
  | jq -r '.access_token')

# 2. Crear Producto
PRODUCTO_ID=$(curl -X POST http://localhost:3012/api/producto \
...
# 3. Subir imagen
curl -X POST http://localhost:3012/api/producto/$PRODUCTO_ID/upload-image \
  -H "Authorization: Bearer $TOKEN" \
...
- La URL debe empezar con `http://localhost:3012/uploads/`
...
```bash
curl -X GET http://localhost:3012/api/cliente-profile/me \
...
```bash
curl -X PUT http://localhost:3012/api/cliente-profile/me \
  -H "Authorization: Bearer {access_token}" \
  -H "Content-Type: application/json" \
  -d '{
...
**Archivo:** `modaurbana-api.postman_collection.json`
...
├── modaurbana-api.postman_collection.json
└── package.json
```
